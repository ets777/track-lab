...
- **Nav button**: The unified `<app-nav-button>` is the single leading toolbar control (`slot="start"`) — it renders the back arrow when `[showBack]` is true, otherwise the burger (`menu="…"`). Back navigation is driven by `NavigationService` tracked history (not `ion-back-button`). Any page reachable via dashboard navigation tiles must show back when `navigationService.previousUrl === '/dashboard'` (pass the page's `showBackButton`), replacing the menu button for that render. Never use `ion-back-button` or a raw `ion-menu-button` directly.
...
- **Adaptive widgets**: Dashboard widgets must adapt to their grid cell via CSS — use `flex`, `min-height: 0`, `overflow: hidden`, and relative sizing. Never hard-code pixel heights that assume a specific screen resolution. If a widget must limit the number of rendered items (e.g. rules checklist), keep the count small enough to fit the smallest expected cell (currently `VISIBLE_COUNT = 3`). Do not add JS logic that tries to measure or predict available height — let CSS constrain the layout.
